...
int func1(char* string_data) {
    ...
    n_count = func3(min_n_required, n_count);
    if(n_count == NOT_FOUND) {
        n_count = 0;
    } else {
        ++n_count;
    }
    return FIND_MIN(FIND_MIN(n_count, e_count / 3), FIND_MIN(i_count, t_count));
}

static int func2(const int *const search_space, const int key) {
...
static int func3(const int *const search_space, const int number) {
    int found_index = func2(search_space, number);
