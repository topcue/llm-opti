...
def compiler_with_optmz_level(target_dir, target, level):
    ...
    cmd_archive = f"{AR_PATH} rcs {target_dir}/libo{level}.a {target_dir}/o{level}.o"
...
def main():
    ...
    for flag in flags:
        ...
        cmd_assemble = f"{GCC_PATH} -S {target_dir}/{target}.c -masm=intel -o {target_dir}/{output}.s -O1 {flag}"
        subprocess.run(cmd_assemble, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, check=True)
        ...
        if identical:
            ...
            cmd_archive = f"{AR_PATH} rcs {target_dir}/lib{output}.a {target_dir}/{output}.o"
